# python gt-gen-vac-randombag.py MSA_NAME RANDOM_SEED quick_test
# python gt-gen-vac-randombag.py Atlanta 66 False
...
# Store filename
filename = os.path.join(gt_result_root, 'randombag_results_%s_%s_%s_%s_%sseeds.csv'%(VACCINATION_RATIO,MSA_NAME,RANDOM_SEED, NUM_GROUPWISE, NUM_SEEDS))
#filename = 'randombag_vaccination_results_withgini_%s_%s_%s_%s_%sseeds.csv'%(VACCINATION_RATIO,MSA_NAME,RANDOM_SEED, NUM_GROUPWISE, NUM_SEEDS)
...
if(os.path.exists(os.path.join(epic_data_root, '3cbg_avg_infect_same_%s.npy'%MSA_NAME))):
    print('cbg_avg_infect_same, cbg_avg_infect_diff: Load existing file.')
    cbg_avg_infect_same = np.load(os.path.join(epic_data_root, '3cbg_avg_infect_same_%s.npy'%MSA_NAME))
    cbg_avg_infect_diff = np.load(os.path.join(epic_data_root, '3cbg_avg_infect_diff_%s.npy'%MSA_NAME))
else:
    print('cbg_avg_infect_same, cbg_avg_infect_diff: File not found. Please check.')
    pdb.set_trace()
    
SEIR_at_30d = np.load(os.path.join(epic_data_root, 'SEIR_at_30d.npy'),allow_pickle=True).item()
...
'''
history_D2_no_vaccination = np.fromfile(os.path.join(epic_data_root,MSA_NAME,'vaccination_results_adaptive_0.1_0.01','20210206_history_D2_no_vaccination_adaptive_0.1_0.01_30seeds_%s'%(MSA_NAME)))
...
'''
result_df = pd.DataFrame(columns=['Vaccinated_Idxs','Final_Cases','Final_Deaths'])
...
for group_idx in range(max_group_idx):
    ...
    for i in range(NUM_GROUPWISE):
        ...
        final_deaths_cbg_randombag = deaths_cbg_randombag[-1,:]

        # Store in df
        result_df = result_df.append({'Vaccinated_Idxs':vaccinated_idxs,
                                      'Final_Cases':final_cases_cbg_randombag,
                                      'Final_Deaths':final_deaths_cbg_randombag
                                    },ignore_index=True)
        print(result_df)  
        result_df.to_csv(os.path.join(gt_result_root,filename))
        ...
        print(result_df)  
        result_df.to_csv(os.path.join(epic_data_root,filename))
        ''' 
