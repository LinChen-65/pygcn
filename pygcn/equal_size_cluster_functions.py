...
def show_chart(coords, ind2cluster, centroids, nclusters, dynamic=True,savepath=None):
  if coords.shape[1] != 2:
    return
  ...
  if(savepath):
        print(f'Save graph at: {savepath}')
...
def cluster_equal_size(coords, nclusters, show_plt=True, savepath=None):
  ...
  while True:
    ...
    if show_plt:
      ...
      show_chart(coords, ind2cluster, centroids, nclusters, savepath=savepath)
...
def cluster_equal_size_pair_split(coords, nclusters, show_plt=True, savepath=None):
...
def cluster_equal_size_swap(coords, nclusters, show_plt=True, savepath=None):
...
def cluster_equal_size_elki(coords, nclusters, show_plt=True, savepath=None):
  clusters, ind2cluster = init_clasters(coords, nclusters)
  old_int2cluster = None
  swap_proposals = [set() for _ in range(nclusters)]
  while True:
...
def cluster_equal_size_detect_cycles(coords, nclusters, show_plt=False, savepath=None):
  clusters, ind2cluster = init_clasters(coords, nclusters)
  old_int2cluster = None
  while True:
    centroids = np.array([sum(coords[l]) / len(l) for l in clusters])
...
def lloid_equal_size_linear_assignment(coords, nclusters, show_plt=True, savepath=None):
  from sklearn.cluster import KMeans
...
def cluster_equal_size_mincostmaxflow(coords, nclusters, show_plt=True, savepath=None):
  clusters, ind2cluster = init_clasters(coords, nclusters)
